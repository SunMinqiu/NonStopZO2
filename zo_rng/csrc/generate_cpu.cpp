/*
 * generate_cpu.cpp — CPU kernel for deterministic normal RNG
 *
 * Uses Random123 Philox4x32-10 for PRNG + deterministic Box-Muller transform.
 * Parallelized via ZoRngThreadPool (dedicated std::thread pool, independent
 * of OpenMP / PyTorch ATen's thread pool).
 */
#include <stdint.h>
...
extern "C"
void zo_rng_generate_cpu(int64_t seed, int64_t counter, int64_t n,
                          float *output, int pool_id) {
    ...
    ZoRngPoolRegistry::instance().get(pool_id).parallel_for(num_blocks, [&](int64_t begin, int64_t end) {
